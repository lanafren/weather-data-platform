with
    source as (
        select
            ...
            datetime(
                timestamp_seconds(cast(json_value(data, '$.sys.sunset') as int64))
            ) as sunset_utc,
            ...
            (
                coalesce(cast(json_value(data, '$.rain.1h') as float64), 0) > 0
            ) as is_raining,
            (
                coalesce(cast(json_value(data, '$.snow.1h') as float64), 0) > 0
            ) as is_snowing,
            fetched_at
    ...
    dedup as (
        select
            *,
            row_number() over (
                partition by record_time_utc order by record_time_utc
            ) as rn
        from source
    )
